...
export async function POST(request: NextRequest) {
  try {
    ...
    // If no Supabase, just log and return success (for local dev)
    if (!supabase) {
      console.log("Telemetry (no DB):", {
        ip,
        events: payload.events.length,
        landing: payload.landing_page,
      });

      // Still notify Slack for assistant messages
      ...
      return new Response(JSON.stringify({
        ...
        session_id: payload.session_id || generateId("sess"),
