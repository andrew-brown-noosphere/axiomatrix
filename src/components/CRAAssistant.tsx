...
export default function CRAAssistant() {
  const [messages, setMessages] = useState<Message[]>([
    {
      role: "assistant",
      content: "What's your role? I'll tailor CRA guidance for you.",
    },
  ]);
  ...
  const sendMessage = useCallback(async (userMessage: string, currentMessages: Message[]) => {
    ...
    try {
      ...
      const response = await fetch("/api/cra-chat", {
        method: "POST",
  ...
  useEffect(() => {
    ...
    window.addEventListener("cra-clarify", handleClarify as EventListener);
    return () => window.removeEventListener("cra-clarify", handleClarify as EventListener);
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        <div>
          <h3 className="font-semibold text-white">CRA Assistant</h3>
          <p className="text-xs text-zinc-500">Ask about the Cyber Resilience Act</p>
        </div>
      </div>
      ...
      <div className="mt-4 pt-4 border-t border-zinc-800">
        ...
        >
          <div>
            <p className="text-sm font-medium text-white">CRA Readiness Assessment</p>
            <p className="text-xs text-zinc-500">Get your compliance score</p>
          </div>
